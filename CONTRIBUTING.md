...
本规范用于个人项目的通用开发协作，目标是保持流程简单，同时让需求、代码修改和提交记录能够相互追踪。

具体项目如果定义了自己的 `CONTRIBUTING.md`，应优先遵循项目级规范，尤其是分支模型、PR 目标分支、CI 和发布流程。
...
2. 根据项目自己的分支策略创建开发分支。
3. 在开发分支中完成修改并提交代码。
4. 创建 Pull Request，目标分支遵循项目自己的分支策略。
...
目标分支、发布分支以及是否允许直接合并到稳定分支，由具体项目自己的 `CONTRIBUTING.md` 或仓库规则决定。
...
极小的拼写、README 文案或仓库元数据修改，可以根据具体项目规则适当简化流程；影响程序行为的修改原则上仍应遵循完整开发流程。
